export default function Cart({ $target, initialState, onRemove }){
    ...
    const renderOption = (option, index) => {
        ...
        return `<li data-index="${index}" class='cartItem'>${productName} - ${option.optionName} | ${basePrice + option.optionPrice}, ${option.ea}개 <button class="remove">X</button></li>`
    }
    this.render = () => {
        const { selectedOptions } = this.state
        $cart.innerHTML = `
        <ul>
            ${Array.isArray(selectedOptions) && selectedOptions.map((option, index) => renderOption(option, index)).join("")}
        </ul>
        <div>
